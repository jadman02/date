...
console.log(obj.type);
    if (obj.type != 'message'){var typetype = obj.type.substring(0, 4);}
    var correctimage;
    var correctname;
    var iconhtml;
    var colordot;
...
//need to see if a match still and then create function based on tha
   ...
   // onclick="singleBrowser('+targetid+')"
    ...
    if (obj.type=='message'){message_text = obj.message;  iconhtml = '<i class="pe-7s-mail" style="margin-right:5px;z-index:9999;"></i>'}
        if (obj.type=='image'){
...
if (rightdates[i].response =='W') {
    ...
    if (rightdates[i].created_uid == f_uid) {confirmtext = 'Waiting for '+rightdates[i].received_name+'\'s response.';}
...
' <div class="toolbar-inner yes-inner" style="display:none;text-align:center;padding-left:0px;padding-right:0px;">'+
 ...
 ' <div class="toolbar-inner date-inner" style="padding-left:0px;padding-right:0px;display:none;text-align:center;">'+
  '<div style="width: calc(100% - 70px); height:44px;background-color:#2196f3;padding-left:5px;" class="link"><textarea id="datemessageq" placeholder="Message (optional)" style="color:white;background-color:#2196f3;margin-top:5px;"></textarea></div>'+
  '<a href="#" class="link" style="color:white;background-color:#2196f3;width:70px;" onclick="dateRequest();"><span style="margin: 0 auto;">Send</span></a>'+
  '</div>'+

 ' <div class="toolbar-inner message-inner" style="display:none;background-color:#2196f3;padding-left:0px;padding-right:0px;">'+
  '<a href="#" class="link icon-only" onclick="showPhotos();"><i class="pe-7s-camera pe-lg" style="color:white;"></i></a> <input type="file" size="70" capture="camera" accept="image/*" class="dealPictureField imagenotchosen" id="takePictureField_" onchange="getPicture();" style="background-color:transparent;color:transparent;float:left;cursor: pointer;height:44px;width:40px;z-index:1;opacity:0;background-color:red;margin-top:-22px;margin-left:-40px;"><textarea id="messagearea" placeholder="Message" style="color:white;background-color:#2196f3;margin-left:5px;"></textarea><a href="#" class="link" style="color:white;margin-right:5px;" onclick="sendMessage();">Send</a>'+
  '</div>'+
'</div>'+  
...
if (d_type == 'date') {$( ".requestbutton" ).text('Request Date');$( ".dateheader" ).text('Date Request');}
if (d_type == 'duck') {$( ".requestbutton" ).text('Request Duck');$( ".dateheader" ).text('Duck Request');}
}
...
'<div class="item-title-row">'+
          ...
          '</div>'+
                  ...
                  '  <div class="item-subtitle" style="text-align:left;">'+d_day+timedisplay+'</div>'+
...
if (d_interest && d_type =='duck'){
    ...
    if ((d_interest == 'my') && (d_created_uid == f_uid)){$( ".interestdiv").append('<i class="pe-7s-map-marker"></i> At '+f_first+'\'s place');}
    if ((d_interest == 'my') && (d_created_uid != f_uid)){$( ".interestdiv").append('<i class="pe-7s-map-marker"></i> At'+targetname+'\'s place');}
        if ((d_interest == 'your') && (d_created_uid == f_uid)){$( ".interestdiv").append('<i class="pe-7s-map-marker"></i> At '+targetname+'\'s place');}
    if ((d_interest == 'your') && (d_created_uid != f_uid)){$( ".interestdiv").append('<i class="pe-7s-map-marker"></i> At '+f_first+'\'s place');}
